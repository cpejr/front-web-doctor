...
function ModalExameMarcado(props) {
  ...
  async function setandoValoresExame() {
    ...
    const res = await managerService.dispostivoById(props.exame.id_dispostivo);


    setConsultorio(resposta.nome);
    setDispositivo(res.titulo);
    setCarregando(false);

    if (props.exame.descricao !== null)
      ...
      {carregando ? (
        ...
        <Caixa>
          ...
          >
            ...
            <TextoInformacoes>
              <b>Data de envio: </b> {dataEnvio.slice(8, 10)}/{dataEnvio.slice(5, 7)}/{dataEnvio.slice(0, 4)}
            </TextoInformacoes>
            <TextoInformacoes>
              <b>Data de devolução: </b> {dataDevolucao.slice(8, 10)}/{dataDevolucao.slice(5, 7)}/{dataDevolucao.slice(0, 4)}
            </TextoInformacoes>
            <TextoInformacoes>
              <b>Data de pagamento: </b> {dataPagamento.slice(8, 10)}/{dataPagamento.slice(5, 7)}/{dataPagamento.slice(0, 4)}
            </TextoInformacoes>
            <TextoInformacoes>
