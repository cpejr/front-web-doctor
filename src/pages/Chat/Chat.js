...
const Chat = () => {
  const { usuarioId, conversas, setConversas, conversaSelecionada, setMensagens } = useContext(ChatContext)
  ...
  const atualizarBarraLateralNovaMensagem = async (novaMensagem) => {
    ...
    const index = conversas?.findIndex(({ id }) => id === novaMensagem.id_conversa)
    ...
    if ((novaMensagem.id_conversa === conversaSelecionada.id)) {
      ...
      await managerService.UpdateMensagemVisualizada(novaMensagem.id, { foi_visualizado: true })
  ...
  return (
    <>
      <Container>
        <BarraLateralChat  />
        {checarObjVazio(conversaSelecionada) ? <MensagemInicial />  : <ConversaAberta socket={socket.current} />}
      </Container>

      <ContainerMobile>
        {checarObjVazio(conversaSelecionada) ? <BarraLateralChat  /> : <ConversaAberta socket={socket.current} />}
      </ContainerMobile>
    </>
  );
};
